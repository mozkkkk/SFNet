...
from utils.losses.loss import LovaszSoftmax, CrossEntropyLoss2d, CrossEntropyLoss2dLabelSmooth,\
    ProbOhemCrossEntropy2d, FocalLoss2d, CriterionICNet
...
def parse_args():
    ...
    # model and dataset
    parser.add_argument('--model', type=str, default="deeplabv3plus_mobilenet", help="model name: (default ENet)")
    parser.add_argument('--dataset', type=str, default="river", help="dataset: cityscapes or camvid")
    parser.add_argument('--input_size', type=str, default="320,800", help="input size of model")
    ...
    parser.add_argument('--isstartinresume', type=bool, default=False, help="is start in resume epoch and lr")
...
def train_model(args):
    ...
    weight = torch.from_numpy(datas['classWeights']).to(torch.float32)

    if args.dataset == 'river' and args.model == 'ICNet':
        criteria = CriterionICNet(weight=weight, ignore_label=ignore_label)
    elif args.dataset == 'camvid' or args.dataset == 'river':
    ...
    if args.dataset == 'river' and args.use_ohem:
        print("ohem")
    ...
    elif args.dataset == 'river' and args.use_label_smoothing:
        print("use_label_smoothing")
        criteria = CrossEntropyLoss2dLabelSmooth(weight=weight, ignore_label=ignore_label)
    elif args.dataset == 'river' and args.use_lovaszsoftmax:
        print("use_lovaszsoftmax")
        criteria = LovaszSoftmax(ignore_index=ignore_label)
    elif args.dataset == 'river' and args.use_focal:
        print("use_focal")
    ...
    if args.use_ohem:        
        args.savedir = (args.savedir + args.dataset + '/' + args.model + 'ohem' + '_bs_'
                    + str(args.batch_size) + '_gpu_' + str(args.gpu_nums) + "_" + str(args.train_type) + '/')
    elif args.use_focal:
        args.savedir = (args.savedir + args.dataset + '/' + args.model + '_focal' + '_bs_'
                    + str(args.batch_size) + '_gpu_' + str(args.gpu_nums) + "_" + str(args.train_type) + '/')
    elif args.use_lovaszsoftmax:
        args.savedir = (args.savedir + args.dataset + '/' + args.model + '_lovaszsoftmax' + '_bs_'
                    + str(args.batch_size) + '_gpu_' + str(args.gpu_nums) + "_" + str(args.train_type) + '/')
    else:
        args.savedir = (args.savedir + args.dataset + '/' + args.model + '_bs_'
                    + str(args.batch_size) + '_gpu_' + str(args.gpu_nums) + "_" + str(args.train_type) + '/')
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if args.dataset == 'camvid':
            ...
            if (epoch > 800 and epoch % 10 == 0) or ((epoch/args.max_epochs)>0.8 and lossTr < last_loss)  or ((epoch/args.max_epochs)>0.8 and epoch%10==0) or (epoch>900 and lossTr < last_loss):
                torch.save(state, model_file_name)
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for iteration, batch in enumerate(train_loader, 0):
        ...
        if args.lr_schedule == 'poly':
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=args.max_epochs, T_mult=2)
        ...
        if args.model == "SFNet":
            ...
            loss = loss + criterion(output[1], labels)
        elif args.model == "LCNet":
            loss = criterion(output, labels)
        elif args.model == "LWGANet":
            loss = criterion(output[0], labels) + criterion(output[1], labels)
        elif "deeplab" in args.model.lower():
            loss = criterion(output, labels)
        else:
            #loss = criterion(output, labels)
            loss = criterion(output[0], labels) + criterion(output[1], labels) + criterion(output[2], labels)
...
def val(args, val_loader, model):
    ...
    for i, (input, label, size, name) in enumerate(val_loader):
        ...
        #if(args.model == 'ICNet' or args.model == 'BiseNet' or args.model == 'BiSeNetGhost'):
        if(args.model == 'model_stages1446'):
            output = output[0]
        if args.model == "SFNet":
            output = output[1]
        if args.model == "FastICENet":
            output = output[0]
        if args.model == "LWGANet":
            output = output
