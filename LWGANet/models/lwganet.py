...
from torch import Tensor, classes
import os
import copy
...
class MRA(nn.Module):
    def __init__(self, channel, att_kernel=7, norm_layer='BN'):
        super().__init__()
        att_padding = att_kernel // 2
        self.gate_fn = nn.Sigmoid()
        self.channel = channel

        # Max pooling to generate the intermediate feature map 'x_tem'
        self.max_m1 = nn.MaxPool2d(kernel_size=3, stride=1, padding=1)
        # Using nn.MaxPool2d as a placeholder for antialiased_cnns.BlurPool
        # The original paper uses BlurPool to prevent aliasing when downsampling.
        self.max_m2 = nn.MaxPool2d(kernel_size=3, stride=3)  # Replaced BlurPool

        # Convolutions for attention branches
    ...
    def forward(self, x):
        # NOTE: The original forward pass had a potential bug:
        # x[:, :self.channel, :, :] implies the input 'x' might have more channels
        # than the module's configured 'channel'. I'll assume x.shape[1] == self.channel.

        x_tem = self.max_m1(x)
        x_tem = self.max_m2(x_tem)

        # Standard attention branches
        x_h1 = self.H_att1(x_tem)
        x_w1 = self.V_att1(x_tem)

        # Transformed attention branches
        ...
        gate = self.gate_fn(att)
        # Upsample attention map to match original input size 'x'
        gate_upsampled = F.interpolate(gate, size=(x.shape[-2], x.shape[-1]), mode='bilinear', align_corners=False)

        out = x * gate_upsampled
        return out

    def h_transform(self, x):
        """Horizontal shearing transform."""
        B, C, H, W = x.shape
        # Pad the width (right side) by H elements.
        x_pad = F.pad(x, (0, H))
        # Reshape to a long strip, then remove the last H elements. This creates the shear.
        x_pad = x_pad.reshape(B, C, -1)[:, :, :-H]
        # Reshape back to a 2D feature map. The new width is (W + H - 1).
        x_transformed = x_pad.reshape(B, C, H, W + H - 1)
        return x_transformed

    def inv_h_transform(self, x):
        """Inverse of the horizontal shearing transform."""
        B, C, H, W_transformed = x.shape
        # Calculate the original width W
        W = W_transformed - H + 1
        # Flatten the transformed tensor
        x_flat = x.reshape(B, C, -1)
        # Pad the flattened tensor to reverse the slicing operation
        x_pad = F.pad(x_flat, (0, H))
        # Reshape to the padded grid format
        x_padded = x_pad.reshape(B, C, H, W + H)
        # Slice to recover the original tensor before the transform's padding
        x_original = x_padded[:, :, :, :W]
        return x_original

    def v_transform(self, x):
        """Vertical shearing transform."""
        # Permute H and W, apply horizontal transform, and permute back.
        x_permuted = x.permute(0, 1, 3, 2)
        x_transformed = self.h_transform(x_permuted)
        return x_transformed.permute(0, 1, 3, 2)

    def inv_v_transform(self, x):
        """Inverse of the vertical shearing transform."""
        # Permute H and W, apply inverse horizontal transform, and permute back.
        x_permuted = x.permute(0, 1, 3, 2)
        x_original = self.inv_h_transform(x_permuted)
        return x_original.permute(0, 1, 3, 2)
