...
class RiverDataSet(data.Dataset):
    """
    ...
    """
    def __init__(self, root='./river', list_path='./river/ImageSets/Segmentation/train.txt',
                 split = 'train',
                 crop_size=(320, 800),
                 mean=(0.485, 0.456, 0.406), scale=True, mirror=True, max_iters=None, ignore_label=255, vars=(0.229, 0.224, 0.225), RGB=True):
        self.root = root
        ...
        self.rgb = RGB
        self.img_ids = [i_id.rstrip() for i_id in open(self.list_path)]
        self.num_classes = 3
        ...
        for name in self.img_ids:
            img_file = osp.join(self.root, 'JPEGImages', name)
            label_file = osp.join(self.root, 'EncodeSegmentationClass', name[:-4] + '.png')
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
...
class RiverTestDataSet(data.Dataset):
    """
    ...
    def __init__(self, root='./river', list_path='./river/ImageSets/Segmentation/test.txt', split = 'test',
                 f_scale=1, mean=(0.485, 0.456, 0.406), ignore_label=255, vars=(0.229, 0.224, 0.225), RGB=True):
        self.root = root
        self.list_path = list_path
        self.ignore_label = ignore_label
        self.mean = mean
...
class RiverTrainInform:
    ...
    """
     # root='./river', list_path='./river/ImageSets/Segmentation'
    def __init__(self, data_dir='./river', classes=3, train_set_file="./river/river_train_list.txt",
                 inform_data_file="./inform/river_inform_1.pkl", normVal=1.10):
        """
        Args:
        ...
        img_ids = [i_id.rstrip() for i_id in open(train_set_file)]
        self.train_set_file = img_ids
    ...
    def readWholeTrainSet(self, fileName, train_flag=True):
        ...
        for name in self.train_set_file:
            # we expect the text file to contain the data in following format
            # <RGB Image> <Label Image>
            # line_arr = line.split()
            # img_file = ((self.data_dir).strip() + '/' + .strip()).strip()
            # label_file = ((self.data_dir).strip() + '/' + line_arr[1].strip()).strip()

            img_file = osp.join(self.data_dir, 'JPEGImages', name)
            label_file = osp.join(self.data_dir, 'EncodeSegmentationClass', name[:-4] + '.png')
...
def main():
    ...
    # dataset = RiverTestDataSet()
    trainLoader = data.DataLoader(
        dataset, batch_size=1, pin_memory=False,
        num_workers=8, shuffle=True, drop_last=True)

    for i, sample in enumerate(trainLoader):
        img, lbl, _, name = sample
        lbl = lbl.long()
        print(img.shape, lbl.shape, name)
if __name__ == '__main__':
    # main()
    dataCollect = RiverTrainInform()
